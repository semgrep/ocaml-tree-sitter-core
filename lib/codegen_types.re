...
let codegen_atom = (atom: B.atom): (string, list((string, B.rule_body))) => {
   switch(atom) {
...
let codegen_simple =  (simple: B.simple): (string, list((string, B.rule_body))) => {
   switch(simple) {
...
let codegen_rule_body = (rule_body: B.rule_body): (string, list((string, B.rule_body))) => {
   switch (rule_body) {
   | B.SIMPLE(simple) => {
   ...
   | B.CHOICE(simples) => {
      ...
      let simple_strs = List.map((simple) => {
         ...
         let new_im_list = [(new_im_type, B.SIMPLE(simple)), ...ims];
...
let codegen_rules = (rules: list(B.rule)): (list((string, list((string, B.rule_body))))) => {
...
let codegen = ((_, rules): B.grammar): (string, list((string, B.rule_body))) => {
  counter := 0;
